        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 10, padding: 36, color: "#fff" }}>
            ...
            <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14 }}>
                {top.map((r, idx) => {
                    ...
                    return (
                        <div key={`${idx}-${r.score}-${r.achieved_at}`} style={{ background: "rgba(0,0,0,0.35)", borderRadius: 16, padding: 10 }}>
                            ...
                            <div style={{ marginTop: 8, fontSize: 13, lineHeight: 1.2 }}>{c ? c.work : r.bestCharId}</div>
            ...
            {ranking.length === 0 && <div style={{ marginTop: 14, opacity: 0.75 }}>記録がありません</div>}
