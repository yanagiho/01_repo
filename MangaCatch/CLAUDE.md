...
## 更新履歴（2026-04-02）Build #99

### フルスクリーン確実化

| 変更内容 | 影響ファイル |
|---------|-------------|
| `kiosk: true` に加え `fullscreen: true` を追加。Windows環境でkioskのみでは全画面にならないケースへの対策 | `electron/main.ts` |

### センターゴーストリング修正

| 修正内容 | 影響ファイル |
|---------|-------------|
| `playerXsNormalized` の初期値が `[0.5]`（センター）だったため、センサー検知開始直後の0→Nスキップフレームでセンター位置が emit されゴーストリングが出ていた → 初期値を `[]` に変更し、0→Nスキップフレームでも明示的に `playerXsNormalized = []` をセット | `SensorManager.ts` |

### アトラクトループ削除

| 変更内容 | 影響ファイル |
|---------|-------------|
| タイトル画面の20秒アトラクトタイマー（→TUTORIAL_VIDEO自動遷移）を完全削除。タイトルはSTARTボタン押下 or センサー検知まで安定ループ。タイトル背景動画が終わるとシーンが切り替わる問題の根本原因だった | `App.tsx` |

### 動画差し替え（2026-04-02）

| 変更内容 | 影響ファイル |
|---------|-------------|
| タイトル背景動画を最新版に差し替え | `public/assets/videos/title_bg.mp4` |
| チュートリアル動画を最新版（14MB）に差し替え | `public/assets/tutorial/tutorial.mp4` |
| カウントダウン動画を最新版に差し替え | `public/assets/videos/countdown.mp4` |

### フォト画面キャラクター下切れ修正

| 修正内容 | 影響ファイル |
|---------|-------------|
| グリッド行の auto 高さ内で `height:"100%"` が解決されず `objectFit:contain` が効かないバグ → キャラコンテナに `calc(100vh - 410px)` の明示高さを設定。`width:100%` + `height:100%` + `objectFit:contain` + `objectPosition:center bottom` の組み合わせでキャラが絶対に切れない構造に修正 | `PhotoScene.tsx` |
